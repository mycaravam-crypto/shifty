...
import { topicIdForRoute } from '@/data/helpTopics'

const auth = useAuthStore()
const router = useRouter()
const route = useRoute()

// Global "Hilfe" entry point (stakeholder-facing German explanations, data/helpTopics.ts) —
// reachable from every page, not just the Dienstplan's own separate keyboard-shortcuts panel.
// Opens pre-selected to the topic matching the page it was opened from.
...
<template>
  <div class="flex min-h-screen">
    ...
    >
      ...
      <div class="text-base font-semibold tracking-tight flex-1">Schichtplaner</div>
      <button
        class="text-slate-300 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none rounded-lg"
    ...
    >
      ...
      <div class="px-3 py-4 border-t border-white/8">
        ...
        <div v-if="auth.claims.email" class="px-3 pb-2 pt-1 text-xs text-slate-500 truncate">
    ...
    <HelpModal
      v-if="showHelp"
      :initial-topic-id="topicIdForRoute(route.name)"
